    ...
    for pak=1:totPak/k
        ...
        for ebno = 1:length(EbNo_Vec)
            ...
            for eq = 1:2;
                ...
                rxFFTZF = fft(rxFFTin,64,1)./repmat(fft(H1',64,1),1,frameCount);
                N0 = 10^(snrs(ebno)/10);
                rxFFTMMSE = fft(rxFFTin,64,1)./(repmat(fft(H1',64,1),1,frameCount)+N0*ones(64,frameCount));
